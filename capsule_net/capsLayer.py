...
class CapsConv(object):
	...
	Args:
		input: A 4-D tensor.
		num_units: integer, the length of the outp vector of a capsule.
		with_routing: boolean
	...
	def __call__(self, input, num_outputs, kernel_size=None, stride=None):
		...
		if not self.with_routing:
			...
			# b_IJ: [1, numb_caps_l, num_caps_l_plus_1, 1]
			...
			# retrurn a tensor with shape [batch_size, 10, 16, 1]
...
def capsule(input, b_IJ, idx_j):
	'''
	The routing algorithm for one capsule on the layer l+1. 

	Args:
		input: A Tensor with [batch_size, num_caps_l_1152, length(u_i)=8, 1]
			   shape, num_caps_l meaning the number of capsules in the layer l. 
	Returns:
	...
	with tf.variable_scope('routing'):
		...
		for r_iter in range(cfg.iter_routing):
			...
			# [16, 1].T x [16, 1] => [1, 1], then reduce the mean in the 
...
def squash(vector):
	...
	Squashing function. 
	Args:
		vector: A 4-D tensor with shape [batch_size, num_casp, vec_len, 1]. 
	Returns:
